...
- **마법진(magic_circle)**: 방향(좌/우) 전환을 아예 하지 않아, 매 타일이 같은
  쪽으로만 꺾이는 나선/원형 경로를 만듭니다. `flip_probability=0`, 반복 허용
  한도를 사실상 무제한으로 설정해 구현했습니다.
...
- **슬로우(enable_slow)**: 곡에서 가장 조용하고 긴 구간(에너지가 낮은 상태가
  2초 이상 지속)을 찾아 그 부분만 템포를 60%로 낮춘 임시 BPM 구간으로 쪼갭니다.
  기존 SetSpeed 메커니즘을 그대로 재사용하므로 별도 검증 없이 타이밍 정확도가
  보장됩니다.
...
전 구간 × 스타일 옵션 5종 조합(마법진/질주맵/슬로우/질주+슬로우/기본) × 시드
20개(총 50,177개 타일 전이)에서 최대 오차 `4.4e-16초`로, 스타일 옵션을 켜도
박자 정확도는 그대로 유지됨을 확인했습니다.
